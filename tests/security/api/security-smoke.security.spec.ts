...
test(
  `CORS rejects requests from unknown origin ${tags(
...
test(
  `Private orders API requires authentication ${tags(
    TAGS.testType.security,
    TAGS.executionScope.critical,
    TAGS.caseStyle.negative,
    TAGS.dataImpact.safe,
...
test(
  `Security headers and session cookie flags are present ${tags(
    TAGS.testType.security,
